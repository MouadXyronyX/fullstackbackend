...
def _enrich_chat(chat: dict, db: SupabaseDB) -> dict:
    ...
    if chat.get("user_id"):
        user = db.get_by_id("users", chat["user_id"], columns="id,name")
        if user:
            chat["user"] = user
    return chat
...
def list_chats(admin=Depends(require_admin), db: SupabaseDB = Depends(get_db)):
    chats = db.get_all("chats", filters={"is_active": f"eq.true"}, order="created_at.desc")
    result = []
    for c in chats:
